...
from utils.baidubce.services import bos
from utils.baidubce.services.bos import bos_handler
from utils.baidubce.services.bos import storage_class
...
class BosClient(BceBaseClient):
    ...
    def _dump_acl_object(acl):
        result = {}
        for k, v in list(acl.__dict__.items()):
            if not k.startswith('_'):
                result[k] = v
        return result

    @required(bucket_name=(str, str), acl=(list, dict))
    ...
    @required(bucket_name=(str, str), canned_acl=str)
    ...
    @required(bucket_name=(str, str), key=str)
    ...
    @required(bucket_name=(str, str), rules=(list, dict))
    ...
    @required(bucket_name=(str, str), cors_configuration=list)
    ...
    @required(bucket_name=(str, str), key=str, file_name=str)
    ...
    def _prepare_object_headers(
        ...
        if user_metadata is not None:
            ...
            for k, v in list(user_metadata.items()):
