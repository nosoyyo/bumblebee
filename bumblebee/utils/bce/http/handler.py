...
import http.client
import json
from utils.bce import bceutils
from utils.bce.exception import BceClientError, BceServerError
...
def parse_json(http_response, response):
    ...
    :type response: bceutils.baidubce.BceResponse
    ...
    if body:
        response.__dict__.update(json.loads(body, object_hook=bceutils.dict_to_python_object).__dict__)
    http_response.close()
    return True
...
def parse_error(http_response, response):
    ...
    :raise bceutils.baidubce.exception.BceClientError: if http status code is NOT 2xx
    """
