...
from conf.weibo import WeiboAccount
...
class WeiboLogin(object):
    ...
    def login(self, user_name=None, pass_word=None):
        """
        login weibo.com, return True or False
        """
        self.user_name = user_name or self.user_name
        self.pass_word = pass_word or self.pass_word
        self.user_uniqueid = None
        self.user_nick = None
        ...
        if not json_data:
            return False
        s_pass_word = self.get_password(
            json_data["servertime"], json_data["nonce"], json_data["pubkey"])
...
.sinaSSOController.feedBackUrlCallBack",
        ...
        if json_data["showpin"] == 1:
            ...
            with open(f"var/captcha/{uuid4()}.jpeg", "wb") as file_out:
